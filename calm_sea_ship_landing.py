...
from mavros_msgs.msg import OverrideRCIn, VFR_HUD, WaypointList, ParamValue
...
def letsland():
    ...
    #### PID constants 
    kp = 0.23 #0.7 #0.28#1 is not good 
    kd = 0.4#1 #0.5  greater than 0.75 not good
    ki = 0.001 #greater than 0.001 not good

    kp_xy = 0.025  #0.05 #0.025 
    kd_xy = 0.09  #0.1  #0.09
    ki_xy = 0.0001  #0.001
    ...
    sim_speed = rospy.ServiceProxy('/mavros/param/set', ParamSet )
    rospy.wait_for_service('/mavros/set_mode')
    rospy.wait_for_service('/mavros/cmd/arming')
    rospy.wait_for_service('/mavros/param/set')

    #resp2 = sim_speed(param_id = "SIM_SPEEDUP", value = param_value)
    ...
    while(landed == 0): 
        ...
        if(time() - time_loop > time_speed):    
            ...
            ship_pitch = (to_rad(float(f_obj.readline()[:-1])))#/15
            print " Ship Pitch :", ship_pitch
            #angle = abs(3)# + (ship_pitch)/4)
            V = sqrt(vx*vx + vy*vy + vz*vz)

            # distance between two points
            dist_land = sqrt((curr_x - (lp_x))*(curr_x - (lp_x)))# + (curr_y - lp_y) *(curr_y - lp_y))
            # desired altitude = dist_from_home * tan (3)
            #print "dist_land", dist_land
            h_des = dist_land*math.tan(to_rad(angle)) + ship_pitch + 5
            ...
            tmp.append(0)
            t.append(h_des)
            #t.append(dist_land*math.tan(to_rad(3))) 
            h.append(height)
            d.append(dist_home)
            ...
            zer.append(0)            
            # 9 things being written into file
            # dist, height, r, p, y, time, position, error, alt, y, yaw_angle
            f_obj1.write(str(dist_home) + "," + str(height) + "," + str(to_deg(roll)) + "," + str(to_deg(pitch)) + "," + str(to_deg(yaw)) + "," + str(time_counter) + "," + str(lp_x) + "," + str(error_alt) + "," + str(ele_angle) + "," + str(curr_y) + "," + str(yaw_angle) + "," + str(pwm_xy - 1500) + "," + str(change_in_roll) + "\n")
            #f_obj1.write(str(curr_x) + " " + str(curr_y) + " " + str(height) + " " + str(psi_local_rad) + " " + str(theta_M) + " " + str(sin(psi_local_rad - theta_M)) + " " + str(error_xy) + " " + str(error_xy_rate) + " " + str(yaw_des) + "\n" )
        ...
        if(((1 < height < 5) and (14 < (curr_x - lp_x) < 15)) or (height < 0.2)):
    ...
    if( (14 < (curr_x - lp_x) < 15) and ( 0 < height < 4) ):
